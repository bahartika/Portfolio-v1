...
const ProjectItem = ({ repo }) => {
  return (
    <div className="flex flex-col justify-between border-2 border-secondary text-secondary w-full p-4 font-quicksand rounded-md hover:-translate-y-2 hover:-translate-x-2 transition-all shadow-md hover:shadow-lg">
      ...
      <div>
        {repo.homepage && (
          <a href={repo.homepage} target="_blank" rel="noopener noreferrer" className="bg-secondary inline-block rounded-full text-primary px-4 py-2 text-sm font-bold hover:bg-primary hover:text-tersier transition">
            Lihat Website
          </a>
        )}
      </div>
